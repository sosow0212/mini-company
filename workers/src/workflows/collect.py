...
실제 외부 소스(RSS/웹 크롤링) 대신 **여러 포맷을 백엔드에 넘기는 경로**를 검증한다 —
이 워커가 하는 일은 "어디서 무슨 포맷으로 가져왔는지 알려주기"까지이고,
파싱·메타추출·청킹·임베딩은 전부 백엔드가 한다(ADR-001).
...
async def run(
    ...
    """레지스트리가 부르는 진입점. 진행 상황은 활동으로, 결과는 요약 문장으로 남긴다."""
    await task.log_info("수집 준비 완료")
    ingested, skipped = await _collect(client, task, employee_id=employee_id)
    await task.log_info("수집 완료")
    logger.info("수집 워크플로우 종료 (신규 %d건, 중복 %d건)", ingested, skipped)
    if ingested == 0 and skipped > 0:
        return "새로 적재할 자료가 없었습니다. 모두 이미 수집된 문서입니다."
    return "자료 수집을 마쳤습니다. 지식 베이스에 적재했습니다."
